...
G4VPhysicalVolume* DMXDetectorConstruction::Construct() {
  ...
  // Filter
  G4Box* solidS = new G4Box("solidS", 0.5*L_t, 0.5*L_t, 0.5*L_t); 
  logicS = new G4LogicalVolume(solidS, leadNCrystal_mat, "logicS");  //sapphireNCrystal_mat                
 ...
 // G4Box* solidSD1 = new G4Box("solidSD1", S_l, S_l, 1*mm); 
  ...
  physSD2 = new G4PVPlacement(0, G4ThreeVector(0.,0.,0.), logicSD2, "physSD2", logicS, false, 0);  //0.5*(P_l+S_l-P_w)
